...
--[[ Local Variables ]]
local pSslParametersClient = {
  caCertPath = "./files/WebEngine/ca-cert.pem",
  certPath = "./files/WebEngine/SelfSigned/client-cert.pem",
  keyPath = "./files/WebEngine/SelfSigned/client-key.pem"
}
...
local function updatepSslParameters(pParam, pValue)
  local updatedpSslParameters = utils.cloneTable(pSslParametersClient)
  updatedpSslParameters[pParam] = pValue
  return updatedpSslParameters
end
...
runner.Title("Test")
for parameter, _ in pairs(pSslParametersClient) do
  runner.Step("Connect WebEngine device", connectWSSWebEngine, { 1, updatepSslParameters(parameter,"" ) })
end
